...
|---|---|---|---|---|
| **1A** · Reglas del framework | Lo que impide desplegar o hace que Appian reclasifique el plug-in en silencio | `scripts/verificar_framework.py` | El contrato, el manifiesto y el **bytecode** de las clases compiladas | `R-F01`–`R-F21` (más `R-F01b`, que reporta bajo `R-F01`). Tres de ellas —`R-F02`, `R-F03`, `R-F08`— corren además en el andamiaje, porque solo necesitan el contrato |
...
| **1B** · Licencias de terceros | Copyleft entre las dependencias: la política más tajante de AppMarket y la única con un absoluto | `scripts/verificar_licencias.py` | El SBOM CycloneDX que genera el propio build (nada de red en tiempo de verificación), y `THIRD_PARTY_NOTICES.md` cotejado contra él | `R-L01`–`R-L03` |
| **1C** · Bundles y locales | El bloqueante número uno: sin el bundle `_en_US` correcto el plug-in **no despliega**. Appian busca el de cada módulo como `<key del plug-in>.<key del módulo>`, así que **cada `<function>` o `<smart-service>` del manifiesto necesita el suyo**, llamado como su `key`. Todos los bundles salen con los acentos escapados a `\uXXXX`, `_en_US` incluido: es el locale del que Appian saca los textos de display, o sea el único cuyo mojibake vería un usuario, y escapar es seguro lea quien lea el `.properties` | `scripts/verificar_bundles.py` | Los `.properties` bajo `src/main/resources`, contra el tipo declarado en el contrato y contra los módulos que declara `appian-plugin.xml` | `R-B01`–`R-B07` |
...
|---|---|
| `.claude-plugin/` | Los dos manifiestos de Claude Code: `plugin.json` (nombre, versión, licencia; el `name` es el prefijo de despacho de los tres agentes) y `marketplace.json` (el repo público como marketplace de sí mismo, para instalar con `/plugin marketplace add raulogm077/appian-plugin-forge`). Un test ata `version` y `description` de los dos |
| `README.md`, `CHANGELOG.md` | Lo primero que lee quien llega: requisitos, instalación, primer uso y solución de problemas; y los cambios por fecha, con lo que afecta a plug-ins ya generados al principio |
| `skills/crear-plugin-appian/` | La skill: `skills/crear-plugin-appian/SKILL.md` —el procedimiento, siete pasos con su criterio de salida— y las cinco referencias que se abren desde él: `skills/crear-plugin-appian/referencias/entrevista.md` (cómo se conduce el paso 1), `skills/crear-plugin-appian/referencias/tipos-de-plugin.md` (cómo se deduce el tipo), `skills/crear-plugin-appian/referencias/certificado.md` (cómo se lee la tabla, qué significa el `STATUS` y cómo se captura la salida del build), `skills/crear-plugin-appian/referencias/entorno-windows.md` (las trampas de PowerShell) y `skills/crear-plugin-appian/referencias/secsp-servlet.md` (el único fallo de build esperado, y por qué). **La mecánica vive en las referencias y el procedimiento en la SKILL**: lo que hay que hacer se lee entero de un tirón, y lo que hay que entender se abre cuando hace falta |
