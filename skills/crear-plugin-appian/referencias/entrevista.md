...
| `descripcion` de cada uno | **Decidido** | Se redacta y se enseña: acaba en el `.properties` que ve el diseñador, como tooltip del campo. En smart services la CLAVE de ese `.properties` no es el nombre del contrato sino el del accesor —`documentoOrigen` da `input.DocumentoOrigen`—, y la compone el andamiaje: no hay que escribirla ni preocuparse por su caja |
